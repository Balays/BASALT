...
from S3_Bins_comparator_within_group_10042023 import *
from S4_Multiple_Assembly_Comparitor_multiple_processes_bwt_10242023 import *
from S5_Outlier_remover_DL_11012023 import *
# from S5_Outlier_remover_DL_10232023 import *
from S6_retrieve_contigs_from_PE_contigs_10302023 import *
from S7_Contigs_retrieve_within_group_10262023 import *
from S7lr_finding_sr_contigs_basing_lr_and_polishing_11022023 import *
from S8_OLC_new_10262023 import *
from S9_Reassembly_10262023 import *
...
def BASALT_main_d(assembly_list, datasets, num_threads, lr_list, hifi_list,
    ...
    if functional_module == 'refinement' or functional_module == 'all':
        ...
        if last_step < 6:
            ...
            if len(assembly_list) != 1:
                ...
                final_binset_comparitor(d_folder, coverage_matrix_list, datasets_fq, num_threads, pwd, 'second_drep')
            f_cp_m=open('Basalt_checkpoint.txt', 'a')
        ...
        if last_step < 8:
            if len(assembly_list) > 1:
                ...
                if len(lr_list) == 0 and len(hifi_list) == 0: ### only HTS datasets presented, perform OLC
                    ...
                    # OLC_main(target_bin_folder, step, bin_comparison_folder, aligned_len_cutoff, similarity_cutoff, coverage_extension, num_threads, ram, mod_bin_folder) ### checkm
                    f_cp_m=open('Basalt_checkpoint.txt', 'a')
    ...
    if cleanup_enabled():
        print('BASALT will remove disposable scratch files while preserving downstream and resume state. Please wait for a little bit longer')
    else:
...
if __name__ == '__main__': 
    ...
    BASALT_main_d(assembly_list, datasets, num_threads, lr_list, hifi_list, hic_list, eb_list, ram, continue_mode, functional_module, sensitive, refinement_paramter, max_ctn, min_cpn, pwd, QC_software, output_folder)
    # BASALT_main(assembly_list, datasets, num_threads, lr_list, ram, continue_mode, functional_module, sensitive, refinement_paramter, hybri_reassembly, max_ctn, min_cpn, pwd)
