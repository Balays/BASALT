...
"""
Step S10 (CheckM2 branch): OLC-based refinement after reassembly.

This module performs an additional round of overlap-layout-consensus
based elongation and comparison on bins generated by previous steps,
further improving contiguity and resolving ambiguous contigs.
"""
...
def reassembly_OLC_main(target_bin_folder, step, bin_comparison_folder,
    ...
    # os.chdir(pwd)
    os.system('cp '+pwd+'/'+target_bin_folder+'_OLC_checkm/quality_report.tsv '+pwd+'/'+target_bin_folder+'_OLC/OLC_quality_report_o.tsv')

    os.chdir(pwd+'/'+target_bin_folder+'_OLC')
    fxyz=open('OLC_quality_report.tsv','w')
    fxyz.write('Bin_ID'+'\t'+'Genome_size'+'\t'+'Completeness'+'\t'+'Contamination'+'\t'+'N50'+'\n')
    for bin_id in best_bin_checkm.keys():
        # fxyz.write(str(bin_id)+'\t'+str(best_bin_checkm[bin_id])+'\n')
        fxyz.write(str(bin_id)+'\t'+str(best_bin_checkm[bin_id]['Genome size'])+'\t'+str(best_bin_checkm[bin_id]['Completeness'])+'\t'+str(best_bin_checkm[bin_id]['Contamination'])+'\t'+str(best_bin_checkm[bin_id]['N50'])+'\n')
    fxyz.close()
