...
def BinAbundance(depth, cov, covgs, output_format, name_of_the_binning_project, path, genome_summary_dict, genome_summary_dict2, gen_sum):
    ...
    f_bin_checkm=open(name_of_the_binning_project+'_quality_report.tsv', 'w')
    f_bin_checkm.write('Bin_ID'+'\t'+'Genome_size'+'\t'+'Completeness'+'\t'+'Contamination'+'\t'+'N50'+'\n')
    n=0
    for line in open(str(name_of_the_binning_project)+'_checkm/quality_report.tsv','r'):
        n+=1
        if n >= 2:
            ...
            checkm[str(binID)]=str(genome_size)+'\t'+str(completeness)+'\t'+str(contamination)+'\t'+str(N50)
            ...
            if str(binID) in new_name_dict.keys():
                ...
                f_bin_checkm.write(str(new_name_dict[str(binID)])+'\t'+str(genome_size)+'\t'+str(completeness)+'\t'+str(contamination)+'\t'+str(N50)+'\n')
            else:
                f_bin_checkm.write(str(binID)+'\t'+str(genome_size)+'\t'+str(completeness)+'\t'+str(contamination)+'\t'+str(N50)+'\n')

    for item in gen_sum:
        if str(item).split('\'')[1] not in checkm.keys():
            checkm[str(item).split('\'')[1]]='0'+'\t'+'0'+'\t'+'0'
            f_bin_checkm.write(name_of_the_binning_project+'_genomes.'+str(genome_id[str(item).split('\'')[1]])+'\t'+'0'+'\t'+'0'+'\t'+'0'+'\t'+'0'+'\n')
    f_bin_checkm.close()
    os.system('cp '+name_of_the_binning_project+'_quality_report.tsv '+str(path)+'_checkm/')
    os.system('mv '+name_of_the_binning_project+'_quality_report.tsv '+path+'_genomes')

    fout=open('prebinned_genomes_output_for_dataframe_'+str(name_of_the_binning_project)+'.txt', 'w')
    fout.write('ID'+'\t'+'Prebinid'+'\t'+'PreviousID'+'\t'+'avgCov'+'\t'+'GenomeSize'+'\t'+'Completeness'+'\t'+'Contamination'+'\t'+'N50'+'\n')
    for item in genome_contig.keys():
...
if __name__ == '__main__': 
    ...
    # }
    # assembly_binning_group={'1':['1_assembly.fasta_700_concoct','1_assembly.fasta_600_concoct']}
    # assembly_binning_group={'1':['1_assembly.fasta_100_semibin']}
    assembly_binning_group={'1':['1_HumanGut_HiFi1-5_cat_assembly.fasta_400_metabat','1_HumanGut_HiFi1-5_cat_assembly.fasta_500_metabat','1_HumanGut_HiFi1-5_cat_assembly.fasta_300_metabat','1_HumanGut_HiFi1-5_cat_assembly.fasta_200_metabat',
                                 '1_HumanGut_HiFi1-5_cat_assembly.fasta_100_semibin','1_HumanGut_HiFi1-5_cat_assembly.fasta_1_SingleContig',
                                 '1_HumanGut_HiFi1-5_cat_assembly.fasta_0.3_maxbin2','1_HumanGut_HiFi1-5_cat_assembly.fasta_0.5_maxbin2','1_HumanGut_HiFi1-5_cat_assembly.fasta_0.7_maxbin2','1_HumanGut_HiFi1-5_cat_assembly.fasta_0.9_maxbin2',
                                 '1_HumanGut_HiFi1-5_cat_assembly.fasta_700_concoct','1_HumanGut_HiFi1-5_cat_assembly.fasta_600_concoct']}
    ...
    PE_connections_files={'1':'condense_connections_HumanGut_HiFi1-5_cat_assembly.fasta.txt'}
    assembly_names={'1':'HumanGut_HiFi1-5_cat_assembly.fasta'}
    num_threads=60
