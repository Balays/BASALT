...
def outlier_predictor(depth_TNF_matrix, contigs_depth, bin_contigs,
    ...
    matrix_path = os.path.join('BestBinset_outlier_test', depth_TNF_matrix)
    if (not os.path.exists(matrix_path)) or os.path.getsize(matrix_path) == 0:
        raise RuntimeError(
            'BestBinset_outlier_test input is empty. '
            'This usually means upstream quality filtering produced no usable bins.'
        )

    exit_code = os.system('ensemble.py')
    if exit_code != 0:
        raise RuntimeError(
            'ensemble.py failed during BASALT outlier prediction. '
            'Inspect the traceback above for the true cause.'
        )
    if (not os.path.exists('Predicted_potential_outlier.txt')) or os.path.getsize('Predicted_potential_outlier.txt') == 0:
        raise RuntimeError(
            'Predicted_potential_outlier.txt was not generated by ensemble.py.'
        )
