            ...
            if 'quality_report.tsv' in str(file) and str(file) != 'Bin_name_mod_quality_report.tsv':
                ...
                for line in open(str(file),'r'):
                    ...
                    if n >= 2:
                        ...
                        genome_size=float(str(line).strip().split('\t')[1].strip())
                        
                        bins_checkm[mod_bin_checkm_name]['Completeness']=completeness
                        bins_checkm[mod_bin_checkm_name]['Genome size']=genome_size
                        bins_checkm[mod_bin_checkm_name]['Contamination']=contamination
                        bins_checkm[mod_bin_checkm_name]['N50']=N50
...
def parse_lr_sam(sam_file, long_read, sn):
    ...
    print('Collecting long reads '+str(sam_file))
    for line in open(sam_file,'r'):
        m2+=1
        flist=str(line).split('\t')
        if len(flist) >= 12:
            ...
            if 'quality_report.tsv' in file:
                ...
                for line in open(file,'r'):
                    ...
                    if n >= 2:
                        ...
                        genome_size=float(str(line).strip().split('\t')[8].strip())
                        completeness=float(str(line).strip().split('\t')[1].strip())
                        contamination=float(str(line).strip().split('\t')[2].strip())
                        N50=float(str(line).strip().split('\t')[6].strip())

                        refined_checkm[str(binID)]={}
                        # refined_checkm[str(binID)]['marker lineage']=str(taxon)
                        refined_checkm[str(binID)]['Completeness']=completeness
                        refined_checkm[str(binID)]['Genome size']=genome_size
                        refined_checkm[str(binID)]['Contamination']=contamination
                        refined_checkm[str(binID)]['N50']=N50
    os.chdir(pwd)
...
def bin_comparison(binset_folder, target_folder, origin_checkm, refined_checkm, pwd, type):
    ...
    for bin_name in refined_checkm.keys():
        ...
        if float(origin_checkm[bin_name_o2]['Contamination']) >= float(refined_checkm[bin_name]['Contamination']):
            ...
            f.write(bin_name_o2+'\t'+str(refined_checkm[bin_name]['Genome size'])+'\t'+str(refined_checkm[bin_name]['Completeness'])+'\t'+str(refined_checkm[bin_name]['Contamination'])+'\t'+str(refined_checkm[bin_name]['N50'])+'\n')
    ...
    for bin_name in origin_checkm.keys():
        if bin_name not in removed_bins.keys():
            ...
            f.write(bin_name+'\t'+str(origin_checkm[bin_name]['Genome size'])+'\t'+str(origin_checkm[bin_name]['Completeness'])+'\t'+str(origin_checkm[bin_name]['Contamination'])+'\t'+str(origin_checkm[bin_name]['N50'])+'\n')
    os.chdir(pwd)
            ...
            if len(datasets_list) != 0:
                ...
                if x == 0:
                    ...
                    if len(contig_seq) >= 1:
                        ...
                        origin_checkm={}
                        os.chdir(pwd+'/'+str(binset_folder))
                        for root, dirs, files in os.walk(pwd+'/'+str(binset_folder)):
                            for file in files:
                                if 'quality_report.tsv' in file:
                                    n=0
                                    for line in open(file,'r'):
                                        ...
                                        if n >= 2:
                                            ...
                                            origin_checkm[str(binID)]={}
                                            # refined_checkm[str(binID)]['marker lineage']=str(taxon)
                                            origin_checkm[str(binID)]['Completeness']=completeness
                                            origin_checkm[str(binID)]['Genome size']=genome_size
                                            origin_checkm[str(binID)]['Contamination']=contamination
                                            origin_checkm[str(binID)]['N50']=N50
                        os.chdir(pwd)
