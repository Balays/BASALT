...
def elongate_contig_selector(eliminated_bin, threshold, pwd, eliminated_bin_containing_folder):
    ...
    if num_contig >= 10:
        ...
        seperate_outlier=outlier_remover(eliminated_bin, contigs_ids, threshold, newData, explained_variance_ratio, pwd)
        ...
        # newData=A[0]
        # explained_variance_ratio=A[1]
        # seperate_outlier=outlier_remover(eliminated_bin, bin_contig_list, threshold, newData, explained_variance_ratio, pwd)
        ...
        for contig in vs_contig_seq.keys():
            f2=open(str(contig)+'_'+eliminated_bin,'w')
            selected_bins.append(str(contig)+'_'+eliminated_bin)
            f2.write('>'+str(contig)+'\n'+str(vs_contig_seq[contig])+'\n')
            f2.close()
        ...
        try:
            if len(merged_files) != 0 and len(recorded_bin2) != 0:
                ...
                if checkmmode == 'tw':
                    ...
                    try:
                        os.system('checkm taxonomy_wf -t '+str(num_threads)+' -x fa '+str(marker_lineage_level_name)+' '+str(marker_lineage_taxon)+' '+target_bin+'_merged '+str(target_bin)+'_checkm')
                    except:
                        xxxxyyyy=0
                    if os.path.exists(pwd+'/'+str(target_bin)+'_checkm/storage/bin_stats_ext.tsv'):
                        print(str(target_bin)+' taxonomy wf done')
...
def merge(target_bin_folder, target_bin, eliminated_bin_list, target_bin_checkm, num, pwd, aligned_len_cutoff, similarity_cutoff, coverage_extension, num_threads, orig_binset, bin_comparison_folder, checkmmode):
    ...
    for item in eliminated_bin_list:
        ...
        if num == 0:
            ...
            if os.path.exists(pwd+'/'+str(item)):
                print(str(item)+' already presented in '+pwd)
            else:
                os.system('cp '+pwd+'/'+eliminated_bin_containing_folder+'/'+str(item)+' '+pwd)
        ...
        n+=1
        if n == 1:
            print('Use '+item+' to elong '+str(target_bin))
...
def mul_threads(item, bestbinset_sim_bin, bestbinset_checkm, step, pwd, aligned_len_cutoff, similarity_cutoff, orig_binset, target_bin_folder, bin_comparison_folder, coverage_extension, num_threads, checkmmode):
    ...
    if os.path.exists(pwd+'/'+target_bin_rename+'/'+str(item)):
        print(str(item)+' already presented in '+pwd+'/'+target_bin_rename)
    else:
        os.system('cp '+pwd+'/'+target_bin_folder+'/'+str(item)+' '+pwd+'/'+target_bin_rename)

    if os.path.exists(pwd+'/'+target_bin_folder+'_temp/'+str(item)):
        print(str(item)+' already presented in '+pwd+'/'+target_bin_folder+'_temp')
    else:
...
if __name__ == '__main__': 
    ...
    num_threads=60
    ram=250
    OLC_main_checkm(target_bin_folder, step, bin_comparison_or_reassembly_binset_folder, aligned_len_cutoff, similarity_cutoff, coverage_extension, num_threads, ram, orig_binset)
