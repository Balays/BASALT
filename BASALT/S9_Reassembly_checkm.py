...
def mod_bin(binset_folder):
    ...
    ----------
    binset_folder : str
        Folder containing the original per-bin FASTA and CheckM
        ``bin_stats_ext.tsv`` output.
    ...
    dict
        Mapping ``bin_id -> quality_metrics`` parsed from the CheckM
        ``bin_stats_ext.tsv`` file.
    """
    pwd=os.getcwd()
    bins_checkm={}
    try:
            ...
            if 'bin_stats_ext.tsv' in str(file) and str(file) != 'Bin_name_mod_bin_stats_ext.tsv':
                for line in open(str(file),'r'):
                    ...
                    try:
                        ...
                        bins_checkm[mod_bin_checkm_name]['Genome size']=float(str(line).strip().split('\'Genome size\':')[1].strip().split(', ')[0].replace('\'','').replace('\"',''))
                    try:
...
def parse_sam(sam_file, fq, pair, n):
    """
    Parse Bowtie2 SAM output and split paired reads by bin (CheckM branch).
...
def parse_lr_sam(sam_file, long_read, sn):
    """
    Group long reads by bin based on SAM alignments (CheckM branch).
...
def re_assembly_main(binset_folder, datasets_list, long_read,
    ...
    """
    Entry point for S9 short-read reassembly (CheckM branch).
    ...
    if x == 0:
        ...
        while xyzzz == 0 and ix <= 2:
            ...
            if ix == 1:
                ...
                os.system('rm -rf '+str(binset_folder)+'_re-assembly_binset_checkm')
                os.system('checkm lineage_wf -t '+str(num_threads)+' -x fa '+str(binset_folder)+'_re-assembly_binset '+str(binset_folder)+'_re-assembly_binset_checkm')

            try:
                xyzzz = 0
                for line in open(pwd+'/'+str(binset_folder)+'_re-assembly_binset_checkm/bin_stats_ext.tsv','r'):
                    xyzzz+=1
            except:
                xyzzz=0
            ...
            if xyzzz != 0:
                ...
                if ix == 1:
                    f=open('Assembly_status.txt','a')
                    f.write('Reassembly CheckM error! Re-do'+'\n')
                    f.close()
                    print('Reassembly CheckM error! Re-do!')
                else:
                    f=open('Assembly_status.txt','a')
                    f.write('Reassembly CheckM error! Abandon!'+'\n')
                    f.close()
                    print('Reassembly CheckM error! Abandon!')
                

    os.chdir(pwd)
    reassembly_bin_checkm=parse_checkm(binset_folder+'_re-assembly_binset_checkm/storage',pwd)
