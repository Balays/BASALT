...
def assembly_mul(bins_seq_folder, bin_seq, item, reassembly_bin_folder,
                 pwd, num_threads, ram):
    """
    Assemble short-read data for a single bin using SPAdes (CheckM branch).
...
def SR_reassembly(bin_seq, reassembly_bin_folder, num_threads,
    ...
    """
    Run short-read-only hybrid reassembly workflow (CheckM branch).
...
def hybrid_re_assembly_main(binset_folder, sr_folder, lr_folder,
                            ram, num_threads):
    """
    Entry point for S9p hybrid reassembly (CheckM branch).
